...
void FirstPersonCamera () {
    glRotatef(xrot,1.0,0.0,0.0);  //rotate our camera on the x-axis
    glRotatef(yrot,0.0,1.0,0.0);  //rotate our camera on the y-axis
    glTranslated(-xpos,-ypos,-zpos); //translate position
}
...
void display()
{
   ...
   Print("Angle=%d,%d Dim=%.1f FOV=%d mode=%d x=%.2f y=%.2f z=%.2f xrot=%.2f yrot=%.2f",th,ph,dim,fov,mode,xpos,ypos,zpos,xrot,yrot);
   //  Render the scene
   glFlush();
...
int main(int argc,char* argv[])
{
   ...
   //  Create the window
   glutCreateWindow("Michael Crozier HW4");
